...
input.addEventListener('onkepress', funcInput);
...
area.addEventListener('click', function (ev) {
  if (ev.target.className == 'box' && ev.target.innerHTML === '') {
    if (player === 'X') {
...
function check() {
  ...
  console.log(boxes);
  if (
    (boxes[0].outerText === 'X' && boxes[1].outerText === 'X' && boxes[2].outerText === 'X') ||
    ...
    (boxes[0].outerText === 'O' && boxes[1].outerText === 'O' && boxes[2].outerText === 'O') ||
